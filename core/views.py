...
# - client-credentials for matching
# - service account for playlist ops
# ---------------------------------------------------------------------
SPOTIFY_TOKEN_URL = "https://accounts.spotify.com/api/token"
SERVICE_USER_ID = os.environ.get("SPOTIFY_SERVICE_USER_ID")  # required for playlist ops
...
# ---------------------------------------------------------------------
# Access request / gating
# ---------------------------------------------------------------------
def request_access(request):
    if request.method == "POST":
        SpotifyAccessRequest.objects.create(
...
def connect_spotify_entry(request):
    ...
    # If they are approved you could redirect into a user-auth flow here
...
@ensure_csrf_cookie
@never_cache
def upload_link(request):  # 👈 no @approved_required anymore
    if request.method == "POST":
        ...
        # Clear previous upload-related session keys
...
def match_report_csv(request, job_id: str):
    ...
    for r in (job.get("results") or []):
        w.writerow([
            r.get("status"),
            r.get("score"),
            smart_str(r.get("raw_title", "")),
            smart_str(r.get("raw_artist", "")),
            smart_str(r.get("raw_album", "")),
            r.get("spotify_id"),
            r.get("method"),
        ])
    return resp
...
# ---------------------------------------------------------------------
# Service account token + API helpers
# ---------------------------------------------------------------------
def _now_ts():
...
# ---------------------------------------------------------------------
# Invite-bot bridge — calls tools/spotify_invite_bot.py
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Internal helper: convert job → collaborative playlist
# ---------------------------------------------------------------------
def _convert_job_to_playlist(job_id: str, playlist_name: str | None = None) -> dict:
    """
    Internal helper: create collaborative playlist for given job_id.
    Returns dict: { invite_link, open_url, spotify_uri, tracks }.
    """
    job = _jobs_col().find_one({"_id": job_id})
    if not job:
        raise ValueError("Job not found")

    playlist_name = (playlist_name or job.get("source_name") or "Imported from Apple Music")[:100]
    ...
    # 6) Also persist onto the job so templates can use it
...
# ---------------------------------------------------------------------
# Convert API - thin wrapper around _convert_job_to_playlist
# ---------------------------------------------------------------------
@csrf_exempt
def api_convert(request):
    ...
    try:
        data = _convert_job_to_playlist(job_id, playlist_name)
    except ValueError as e:
        return JsonResponse({"error": str(e)}, status=404)
    except Exception as e:
        return JsonResponse({"error": f"Failed to convert: {e}"}, status=500)

    return JsonResponse(data)


# ---------------------------------------------------------------------
# Results page – auto-creates playlist if needed
# ---------------------------------------------------------------------
@require_GET
@never_cache
def match_results_page(request, job_id: str):
    ...
    # Try to read existing playlist URL from job
    playlist_url = job.get("playlist_url") or job.get("invite_link") or job.get("open_url")

    # If none yet, auto-create playlist now (no internal HTTP calls)
    if not playlist_url:
        try:
            data = _convert_job_to_playlist(job_id, job.get("source_name"))
            playlist_url = data.get("invite_link") or data.get("open_url")
        except Exception as e:
            print("AUTO-CONVERT ERROR:", e)
            # leave playlist_url as None/empty; UI will just not show the button

    # Allow querystring override for backwards-compat
...
# ---------------------------------------------------------------------
# Regenerate invite link for an existing playlist
# ---------------------------------------------------------------------
@csrf_exempt
@require_POST
