...
# Recognize Apple Music playlist URLs (including geo.apple.com)
...
def _extract_song_urls_from_playlist_html(html_text: str):
    """
    Primary: <meta property="music:song" content="https://music.apple.com/.../song?i=ID">
    Fallbacks can be added if needed (JSON-LD, og tags).
    """
...
def _extract_track_id_from_song_url(url: str):
    """
    Prefer ?i=<trackId>. Fallback to last numeric path segment.
    """
...
def _lookup_tracks_itunes(ids):
    """
    Use iTunes Lookup API for metadata. Accepts up to ~200 IDs comfortably.
    """
    if not ids:
        return {}
    ids_csv = ",".join(ids)
...
def parse_apple_playlist_from_url(url: str):
    ...
    Plus these are compatible with views._shrink_track which also exposes aliases.
    """
    ...
    rows = []
    for tid in ids:
        meta = itunes_map.get(tid, {}) or {}
        title  = meta.get('trackName', '') or ''
        artist = meta.get('artistName', '') or ''
        album  = meta.get('collectionName', '') or ''
        isrc   = meta.get('isrc', '') or ''
        dur_ms = meta.get('trackTimeMillis')  # may be None
        ...
        rows.append({
            ...
            'duration_ms': dur_ms,
            # Optional aliases if you want the parser to be self-sufficient:
            'title': title,
            'artist': artist,
            'album': album,
            'isrc': isrc,
